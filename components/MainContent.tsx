    ...
    <div className="relative flex space-x-2 pt-4">
      <div className="w-1/4 sm:w-1/6 h-44 rounded-xl shadow overflow-hidden flex flex-col group cursor-pointer">
        ...
        <div className="flex-1 relative flex items-end justify-center pb-2 text-center leading-none dark:bg-dark-second dark:text-dark-txt">
          ...
          <div className="w-10 h-10 rounded-full bg-teal-500 text-white grid place-items-center text-2xl border-4 border-white dark:border-dark-second absolute -top-5 left-1/2 transform -translate-x-1/2">
            <AddIcon />
          </div>
        </div>
      </div>
      ...
      {suggestions.map((data, index) => (
        ...
        >
          <div className="relative h-full group cursor-pointer">
            ...
            />
            <div className="w-full h-full bg-black absolute top-0 left-0 bg-opacity-10"></div>
            <span className="absolute bottom-0 left-2 pb-2 font-semibold text-white group-hover:transform group-hover:scale-110 transition-all duration-700">
              {data.username}
            </span>
